...
| `routing` | 管理路网、道路空间索引、搜索期硬禁边、六环通行口拓扑、单调跨界约束、多目标规划和版本化路由快照 | `GraphHopperManager`、`SixthRingRoutePlanner`、`EdgeKeyMultiTargetDijkstra`、`BoundaryTraversalConstraint`、`SixthRingPortalTopologyBuilder`、`RoutingSnapshotManager` |
...
| `SixthRingRoutePlanner` | 自动判定四种规划模式；跨界时叠加方向相关的边界遍历约束，运行正向/反向多目标搜索，再用合规完整参考路线决胜 |
| `EdgeKeyMultiTargetDijkstra` | 使用到达 edge key 作为状态，支持 turn costs、禁行边、反向搜索、QueryGraph 虚拟边、通行口 edge 前缀约束和资源上限 |
| `BoundaryTraversalConstraint` | 在搜索扩展阶段执行“留在指定 Polygon”或“不得进入内核”的空间硬约束 |
| `TraversalConstrainedWeighting` | 将每请求边界约束包装进 GraphHopper Weighting，不改变无约束场景 |
| `SixthRingRouteShapeValidator` | 在候选拼接后用 JTS 独立确认安全段覆盖关系和参考段不重返内核 |
| `PortalAnchoredRouteFinder` | 从已选通行口的真实有向 edge 拓扑继续搜索参考段，禁止按相同平面坐标跳到桥下或桥上道路 |
| `NavigationHandoffSelector` | 使用参考段和安全段的 `road_class`、`road_class_link`、`road_environment` 轨迹选择可导航普通道路点；入环路线在边界处仍为高速/匝道时选择安全段中的真实驶离点，高速主线、连接匝道、隧道和轮渡均不暴露为端点 |
| `ExternalHandoffSelector` | 从参考段选择严格位于外边界之外的位置描述解析点，并计算不会跨界的 POI 搜索半径 |
...
POST /api/v1/routes
  ...
  -> INTERNAL_SAFE：GraphHopperRoutingEngine 全局搜索，不施加六环空间限制
  -> 跨界路线：QueryGraph + EdgeKeyMultiTargetDijkstra 在安全侧 Polygon 内搜索 Dmin + 1000 米通行口
  -> 通行口位于 edge 内部时只允许遍历到通行口的前缀
  -> 对候选通行口生成不得进入六环内核的参考路线
  -> SixthRingRouteShapeValidator 独立检查跨界单调形状
  -> 按完整距离、时长、portal ID 决胜
  -> 从参考段与安全段道路轨迹选择 navigationHandoff，并从环外参考段选择 externalHandoff
  -> navigationHandoff 非空时在该点切分 safeSegment / referenceSegment
  -> RouteSafetyValidator 独立检查完整 geometry 的摄像头冲突
  -> 输出转换为 GCJ-02
  -> RouteResponse(技术边界点 + 可选道路交接点 + 外部审计点 + 分段路线 + 完整 geometry，cameraConflictCount = 0)
```

`BlockedEdgeWeighting` 在搜索阶段拒绝禁行基础边及其 QueryGraph 虚拟边，`TraversalConstrainedWeighting` 只在跨界子搜索中叠加空间约束。最终摄像头和路线形状 JTS 校验都是第二道检查，不能替代搜索期硬约束。`EXTERNAL_ONLY` 没有 `safeSegment`，只返回完整外部参考路线；边界带点位在规则获批前返回独立歧义错误。
...
RouteSearchPanel
  ...
  -> 围绕 navigationHandoff 在 200 米内优先解析交通地标，提供复制地标和精确高德导航
  -> externalHandoff 仅保留为严格环外审计/兼容数据，不参与用户可见位置解析
```
