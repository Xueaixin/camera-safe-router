...
| `api.model` | 与 OpenAPI 对应的输入输出数据，不放业务算法 | `RouteRequest`、`RouteResponse`、`CameraUpdateResult` |
...
| `routing` | 管理路网、道路空间索引、搜索期硬禁边和版本化路由快照 | `GraphHopperManager`、`GraphHopperRoutingEngine`、`BlockedEdgeGenerator`、`RoutingSnapshotManager` |
...
| 对象 | 生命周期和作用 |
|---|---|
| `GraphHopperManager` | 进程内单例；加载或构建 GraphHopper 图，持有道路索引和图指纹 |
...
flowchart LR
    ...
    E --> F["RoutingSnapshotManager.refreshNow"]
    F --> G["读取 cameras/camera.json"]
    G --> H["过滤并匹配道路"]
    H --> I["持久化并发布 RoutingSnapshot"]
    I --> J["readiness = READY"]
```
...
POST /api/v1/routes
  ...
  -> 输入坐标统一转换为 WGS84
  -> 检查路网范围和起终点限制
  -> GraphHopperRoutingEngine 使用当前禁行边搜索
  -> RouteSafetyValidator 独立检查路线几何
  -> 输出转换为 GCJ-02
  -> RouteResponse(cameraConflictCount = 0)
```

`BlockedEdgeWeighting` 在搜索阶段拒绝禁行基础边及其 QueryGraph 虚拟边。最终 JTS 校验是第二道安全检查，不能用它代替搜索期硬禁边。
